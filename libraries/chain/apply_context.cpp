    ...
    namespace chain {
        ...
        void apply_context::exec_one(action_trace &trace) {
            ...
            try {
                ...
                } FC_RETHROW_EXCEPTIONS(warn, "pending console output: ${console}",
 ...
 */
        void apply_context::execute_inline(action &&a) {
            ...
            for (const auto &auth : a.authorization) {
                ...
                EOS_ASSERT(control.get_authorization_manager().find_permission(auth) != nullptr,
            ...
            if (!control.skip_auth_check() && !privileged && a.account != receiver) {
                control.get_authorization_manager()
                        .check_authorization({a},
                                             {},
                                             {{receiver, config::celes_code_name}},
        ...
        void
        apply_context::schedule_deferred_transaction(const uint128_t &sender_id, account_name payer, transaction &&trx,
            ...
            trx.expiration = control.pending_block_time() + fc::microseconds(
                    999'999); // Rounds up to nearest second (makes expiration check unnecessary)
            ...
            uint32_t trx_size = 0;
            if (auto ptr = db.find<generated_transaction_object, by_sender_id>(
                    boost::make_tuple(receiver, sender_id))) {
                EOS_ASSERT(replace_existing, deferred_tx_duplicate,
                ...
                db.modify<generated_transaction_object>(*ptr, [&](auto &gtx) {
                    ...
                    gtx.expiration = gtx.delay_until + fc::seconds(
                            control.get_global_properties().configuration.deferred_trx_expiration_window);
            ...
            EOS_ASSERT(control.is_ram_billing_in_notify_allowed() || (receiver == act.account) || (receiver == payer) ||
                       privileged,
        ...
        const table_id_object &
        apply_context::find_or_create_table(name code, name scope, name table, const account_name &payer) {
        ...
        int apply_context::db_store_i64(uint64_t scope, uint64_t table, const account_name &payer, uint64_t id,
                                        const char *buffer, size_t buffer_size) {
            return db_store_i64(receiver, scope, table, payer, id, buffer, buffer_size);
        }

        int apply_context::db_store_i64(uint64_t code, uint64_t scope, uint64_t table, const account_name &payer,
                                        uint64_t id, const char *buffer, size_t buffer_size) {
//   require_write_lock( scope );
            const auto &tab = find_or_create_table(code, scope, table, payer);
            auto tableid = tab.id;

            EOS_ASSERT(payer != account_name(), invalid_table_payer,
...
/// CELES code：hubery.zhang {@
        bool apply_context::verify_wood(uint32_t block_number, const account_name &account, const char *wood) const {
            static celesos::forest::forest_bank *forest = celesos::forest::forest_bank::getInstance(
                    apply_context::control);
