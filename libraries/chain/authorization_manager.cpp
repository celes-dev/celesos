...
namespace eosio { namespace chain {
                                                                             ...
                                                                             )const
   {
      ...
      try {
         ...
         if( *linked_permission == config::celes_any_name )
            return optional<permission_name>();
                                                           ...
                                                           )const
   {
      ...
      if( !linked_permission_name ) // if action is linked to celes.any permission
         return;
                                                             ...
                                                             )const
   {
      ...
      if( *unlinked_permission_name == config::celes_any_name )
         return;
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is celes.any
