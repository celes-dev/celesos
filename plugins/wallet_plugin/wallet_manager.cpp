...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple kcelesd are in this function at once.
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      EOS_THROW(wallet_exception, "Failed to lock access to wallet directory; is another kcelesd running?");
   }
}
