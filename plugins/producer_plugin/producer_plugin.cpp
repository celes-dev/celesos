   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("kcelesd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
...
static producer_plugin_impl::signature_provider_type
make_kcelesd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kcelesd_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kcelesd_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), fc::optional<uint16_t>());
   else
      kcelesd_url = fc::url(url_str);
   ...
   return [weak_impl, kcelesd_url, pubkey]( const chain::digest_type& digest ) {
      ...
      if (impl) {
         ...
         auto deadline = impl->_kcelesd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kcelesd_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kcelesd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            if (spec_type_str == "KEY") {
               ...
               my->_signature_providers[pubkey] = make_kcelesd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kcelesd_provider_timeout_us = fc::milliseconds(options.at("kcelesd-provider-timeout").as<int32_t>());
