...
const char* error_advice_public_key_type_exception = R"=====(Public key should be encoded in base58 and starts with CELES prefix)=====";
...
const char* error_advice_authority_type_exception = R"=====(Ensure that your authority JSON is valid follows the following format!
{
  ...
  "keys": [         <keys must be alpha-numerically sorted by their string representations and unique>
    ...
    {
      "key":        <STRING: CELES.OS compatible Public Key>,
...
const char* error_advice_tx_duplicate =  "You can try embedding celes nonce action inside your transaction to ensure uniqueness.";

const char* error_advice_invalid_action_args_exception = R"=====(Ensure that your arguments follow the contract abi!
You can check the contract's abi by using 'clceles get code' command.)=====";
...
If you are currently using 'clceles push action' command, try to add the relevant authority using -p option.)=====";
