...
//copy pasta from kcelesd's main.cpp
...
const fc::microseconds abi_serializer_max_time = fc::seconds(
        10); // No risk to client side serialization taking a long time
...
void add_standard_transaction_options(CLI::App *cmd, string default_permission = "") {
    ...
    cmd->add_flag("-f,--force-unique", tx_force_unique, localized(
    ...
    cmd->add_flag("--return-packed", tx_return_packed,
                  localized("used in conjunction with --dont-broadcast to get the packed transaction"));
    cmd->add_option("-r,--ref-block", tx_ref_block_num_or_id, (localized(
    ...
    cmd->add_option("--max-cpu-usage-ms", tx_max_cpu_usage, localized(
    ...
    cmd->add_option("--max-net-usage", tx_max_net_usage, localized(
...
fc::variant call(const std::string &url,
                 ...
                 const T &v) {
    try {
        auto sp = std::make_unique<eosio::client::http::connection_param>(context, parse_url(url) + path,
                                                                          no_verify ? false : true, headers);
        return eosio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response);
    }
    catch (boost::system::system_error &e) {
        if (url == ::url)
            std::cerr << localized("Failed to connect to node at ${u}; is nodceles running?", ("u", url)) << std::endl;
        else if (url == ::wallet_url)
            std::cerr << localized("Failed to connect to kcelesd at ${u}; is kcelesd running?", ("u", url)) << std::endl;
...
fc::variant push_actions(std::vector<chain::action> &&actions, int32_t extra_kcpu,
                         packed_transaction::compression_type compression = packed_transaction::none) {
...
void print_action(const fc::variant &at) {
   ...
   */
    ...
    cout << "#" << std::setw(14) << right << receiver << " <= " << std::setw(28) << std::left << (code + "::" + func)
         << " " << args << "\n";
    if (console.size()) {
...
auto abi_serializer_resolver = [](const name &account) -> optional<abi_serializer> {
    ...
    if (it == abi_cache.end()) {
        ...
        if (abi_results.abi.valid()) {
            ...
            std::cerr << "ABI for contract " << account.to_string()
...
bytes json_or_file_to_bin(const account_name &account, const action_name &action, const string &data_or_filename) {
    ...
    if (!data_or_filename.empty()) {
        ...
        } EOS_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'",
...
void send_actions(std::vector<chain::action> &&actions, int32_t extra_kcpu = 1000,
                  packed_transaction::compression_type compression = packed_transaction::none) {
...
void send_transaction(signed_transaction &trx, int32_t extra_kcpu,
                      packed_transaction::compression_type compression = packed_transaction::none) {
...
chain::action
create_newaccount(const name &creator, const name &newaccount, public_key_type owner, public_key_type active) {
    return action{
            tx_permission.empty() ? vector<chain::permission_level>{{creator, config::active_name}}
...
chain::action
create_action(const vector<permission_level> &authorization, const account_name &code, const action_name &act,
...
chain::action create_buyram(const name &creator, const name &newaccount, const asset &quantity) {
    ...
    return create_action(tx_permission.empty() ? vector<chain::permission_level>{{creator, config::active_name}}
                                               : get_account_permissions(tx_permission),
...
chain::action
create_delegate(const name &from, const name &receiver, const asset &net, const asset &cpu, bool transfer) {
    fc::variant act_payload = fc::mutable_variant_object()
            ("from", from.to_string())
            ...
            ("transfer", transfer);
    return create_action(tx_permission.empty() ? vector<chain::permission_level>{{from, config::active_name}}
                                               : get_account_permissions(tx_permission),
                         config::system_account_name, N(delegatebw), act_payload);
}

fc::variant
regproducer_variant(const account_name &producer, const public_key_type &key, const string &url, uint16_t location) {
...
chain::action create_open(const string &contract, const name &owner, symbol sym, const name &ram_payer) {
    auto open_ = fc::mutable_variant_object
            ...
            ("ram_payer", ram_payer);
    return action{
            tx_permission.empty() ? vector<chain::permission_level>{{ram_payer, config::active_name}}
                                  : get_account_permissions(tx_permission),
...
chain::action
create_transfer(const string &contract, const name &sender, const name &recipient, asset amount, const string &memo) {
    ...
    return action{
            tx_permission.empty() ? vector<chain::permission_level>{{sender, config::active_name}}
                                  : get_account_permissions(tx_permission),
...
chain::action create_setabi(const name &account, const bytes &abi) {
    return action{
            tx_permission.empty() ? vector<chain::permission_level>{{account, config::active_name}}
                                  : get_account_permissions(tx_permission),
            setabi{
                    .account   = account,
                    .abi       = abi
...
create_updateauth(const name &account, const name &permission, const name &parent, const authority &auth) {
    return action{tx_permission.empty() ? vector<chain::permission_level>{{account, config::active_name}}
                                        : get_account_permissions(tx_permission),
...
authority parse_json_authority(const std::string &authorityJsonOrFile) {
    ...
    } EOS_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'",
                             ("data", authorityJsonOrFile))
}

//TODO: maybe rename to celesos
authority parse_json_authority_or_key(const std::string &authorityJsonOrFile) {
    if (boost::istarts_with(authorityJsonOrFile, "EOS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
        ...
        } EOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}",
                                 ("public_key", authorityJsonOrFile))
    } else {
        auto result = parse_json_authority(authorityJsonOrFile);
        EOS_ASSERT(eosio::chain::validate(result), authority_type_exception,
...
asset to_asset(account_name code, const string &s) {
    ...
    if (it == cache.end()) {
        ...
        if (obj_it != obj.end()) {
            ...
            EOS_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}",
                      ("s", sym_str)("c", code));
        }
    }
...
struct set_account_permission_subcommand {
    ...
    set_account_permission_subcommand(CLI::App *accountCmd) {
        auto permissions = accountCmd->add_subcommand("permission",
                                                      localized("set parameters dealing with account permissions"));
        ...
        permissions->add_option("authority", authorityJsonOrFile, localized(
                "[delete] NULL, [create/update] public key, JSON string, or filename defining the authority"))->required();
        permissions->add_option("parent", parentStr, localized(
        ...
        permissions->set_callback([this] {
            ...
            if (is_delete) {
                ...
                if (parentStr.size() == 0 && permissionStr != "owner") {
                    ...
                    const auto account_result = call(get_account_func,
                                                     fc::mutable_variant_object("account_name", accountStr));
...
struct set_action_permission_subcommand {
    ...
    set_action_permission_subcommand(CLI::App *actionRoot) {
        ...
        permissions->add_option("account", accountStr,
                                localized("The account to set/delete a permission authority for"))->required();
        permissions->add_option("code", codeStr,
                                localized("The account that owns the code for the action"))->required();
...
void try_local_port(const string &lo_address, uint16_t port, uint32_t duration) {
    ...
    while (!local_port_used(lo_address, port)) {
        if (duration_cast<std::chrono::milliseconds>(system_clock::now().time_since_epoch()).count() - start_time >
            duration) {
            std::cerr << "Unable to connect to kcelesd, if kcelesd is running please kill the process and try again.\n";
            throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kcelesd")});
        }
    }
}

void ensure_kcelesd_running(CLI::App *app) {
    if (no_auto_kcelesd)
        return;
    // get, version, net do not require kcelesd
    ...
    for (const auto &addr: resolved_url.resolved_addresses)
        if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by kcelesd
            return;

    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running clceles like this: ./clceles ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clceles and kcelesd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kcelesd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":"
                  << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start kcelesd because kcelesd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
    ...
    register_producer_subcommand(CLI::App *actionRoot) {
        ...
        register_producer->add_option("account", producer_str,
                                      localized("The account to register as a producer"))->required();
        register_producer->add_option("producer_key", producer_key_str,
                                      localized("The producer's public key"))->required();
        register_producer->add_option("url", url, localized("url where info about producer can be found"), true);
        register_producer->add_option("location", loc,
                                      localized("relative location for purpose of nearest neighbor scheduling"), true);
        ...
        register_producer->set_callback([this] {
            ...
            } EOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}",
                                     ("public_key", producer_key_str))

            auto regprod_var = regproducer_variant(producer_str, producer_key, url, loc);
            send_actions(
                    {create_action({permission_level{producer_str, config::active_name}}, config::system_account_name,
                                   N(regproducer), regprod_var)});
        });
    }
};
...
struct create_account_subcommand {
    ...
    create_account_subcommand(CLI::App *actionRoot, bool s) : simple(s) {
        auto createAccount = actionRoot->add_subcommand((simple ? "account" : "newaccount"), localized(
                "Create an account, buy ram, stake for bandwidth for the account"));
        createAccount->add_option("creator", creator,
                                  localized("The name of the account creating the new account"))->required();
        createAccount->add_option("name", account_name, localized("The name of the new account"))->required();
        createAccount->add_option("OwnerKey", owner_key_str,
                                  localized("The owner public key for the new account"))->required();
        ...
        if (!simple) {
            ...
            createAccount->add_option("--buy-ram-kbytes", buy_ram_bytes_in_kbytes,
                                      (localized(
                                              "The amount of RAM bytes to purchase for the new account in kibibytes (KiB), default is 8 KiB")));
        ...
        createAccount->set_callback([this] {
            ...
            } EOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}",
                                     ("public_key", owner_key_str));
            try {
                active_key = public_key_type(active_key_str);
            } EOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}",
                                     ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
                if (buy_ram_cchn.empty() && buy_ram_bytes_in_kbytes == 0) {
                    std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required"
                              << std::endl;
                    return;
                }
                action buyram = !buy_ram_cchn.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_cchn))
                                                     : create_buyrambytes(creator, account_name,
...
struct get_transaction_id_subcommand {
    ...
    get_transaction_id_subcommand(CLI::App *actionRoot) {
        auto get_transaction_id = actionRoot->add_subcommand("transaction_id",
                                                             localized("Get transaction id given transaction object"));
        get_transaction_id->add_option("transaction", trx_to_check, localized(
        ...
        get_transaction_id->set_callback([&] {
            ...
            } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'",
                                     ("data", trx_to_check))
        });
    }
};
...
struct vote_producer_subcommand {
    ...
    vote_producer_subcommand(CLI::App *actionRoot) {
        auto vote_producer = actionRoot->add_subcommand("voteproducer", localized("Vote for a producer"));
        vote_producer->add_option("wood", wood, localized("The wood for burn"))->required();
        vote_producer->add_option("block_number", block_number, localized("The block number for the wood"))->required();
        vote_producer->add_option("producer", producer_str, localized("Vote for a producer"))->required();
        vote_producer->add_option("voter", voter_str, localized("The voting account"))->required();
        vote_producer->add_option("wood_owner", wood_owner_str, localized("The wood owner account"));
        add_standard_transaction_options(vote_producer);

        vote_producer->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
                    ("voter_name", voter_str)
                    ("wood_owner_name", wood_owner_str.length() ? wood_owner_str : voter_str)
                    ("wood", wood)
                    ("block_number", block_number)
                    ("producer_name", producer_str);
            std::vector<chain::action> acts{
                    create_action({permission_level{voter_str, config::active_name}}, config::system_account_name,
                                  N(voteproducer), act_payload)};
            send_actions(std::move(acts));
        });
    }
};

struct set_proxy_subcommand {
    ...
    set_proxy_subcommand(CLI::App *actionRoot) {
        ...
        set_proxy->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
                    ("voter_name", voter_str)
                    ("proxy_name", proxy_str);
            send_actions({create_action({permission_level{voter_str, config::active_name}}, config::system_account_name,
                                        N(setproxy), act_payload)});
        });
    }
};
...
struct delegate_bandwidth_subcommand {
    ...
    delegate_bandwidth_subcommand(CLI::App *actionRoot) {
        ...
        delegate_bandwidth->add_option("from", from_str,
                                       localized("The account to delegate bandwidth from"))->required();
        delegate_bandwidth->add_option("receiver", receiver_str,
                                       localized("The account to receive the delegated bandwidth"))->required();
        delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount,
                                       localized("The amount of CCHN to stake for network bandwidth"))->required();
        delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount,
                                       localized("The amount of CCHN to stake for CPU bandwidth"))->required();
        ...
        delegate_bandwidth->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
                    ...
                    ("transfer", transfer);
            std::vector<chain::action> acts{
                    create_action({permission_level{from_str, config::active_name}}, config::system_account_name,
                                  N(delegatebw), act_payload)};
...
struct undelegate_bandwidth_subcommand {
    ...
    undelegate_bandwidth_subcommand(CLI::App *actionRoot) {
        ...
        undelegate_bandwidth->add_option("receiver", receiver_str,
                                         localized("The account to undelegate bandwidth from"))->required();
        undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized(
                "The amount of CCHN to undelegate for network bandwidth"))->required();
        undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount,
                                         localized("The amount of CCHN to undelegate for CPU bandwidth"))->required();
        ...
        undelegate_bandwidth->set_callback([this] {
            ...
            send_actions({create_action({permission_level{from_str, config::active_name}}, config::system_account_name,
                                        N(undelegatebw), act_payload)});
        });
    }
};
...
struct bidname_subcommand {
    ...
    bidname_subcommand(CLI::App *actionRoot) {
        ...
        bidname->set_callback([this] {
            ...
            send_actions(
                    {create_action({permission_level{bidder_str, config::active_name}}, config::system_account_name,
                                   N(bidname), act_payload)});
        });
    }
};
        ...
        list_producers->set_callback([this] {
            ...
            for (auto &row : result.rows) {
                ...
                std::cout << std::left << std::setw(18) << "bidname:" << std::right << std::setw(24)
                          ...
                          << (bid > 0 ? bid : -bid) << "\n"
                          << std::left << std::setw(18) << "last bid time:" << std::right << std::setw(24)
                          << ((std::string) time).c_str() << std::endl;
    ...
    list_bw_subcommand(CLI::App *actionRoot) {
        ...
        list_bw->set_callback([this] {
            ...
            if (!print_json) {
                auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
                if (!res.rows.empty()) {
                    std::cout << std::setw(13) << std::left << "Receiver" << std::setw(21) << std::left
                              << "Net bandwidth"
...
struct buyram_subcommand {
    ...
    buyram_subcommand(CLI::App *actionRoot) {
        ...
        buyram->add_option("amount", amount, localized(
                "The amount of EOS to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
    ...
    sellram_subcommand(CLI::App *actionRoot) {
        ...
        sellram->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
                    ...
                    ("bytes", amount);
            send_actions(
                    {create_action({permission_level{receiver_str, config::active_name}}, config::system_account_name,
                                   N(sellram), act_payload)});
        });
    }
};
...
struct claimrewards_subcommand {
    ...
    claimrewards_subcommand(CLI::App *actionRoot) {
        ...
        claim_rewards->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
                    ("owner", owner);
            send_actions({create_action({permission_level{owner, config::active_name}}, config::system_account_name,
                                        N(claimrewards), act_payload)});
        });
    }
};
...
struct unregproxy_subcommand {
    ...
    unregproxy_subcommand(CLI::App *actionRoot) {
        auto unregister_proxy = actionRoot->add_subcommand("unregproxy",
                                                           localized("Unregister an account as a proxy (for voting)"));
...
struct canceldelay_subcommand {
    ...
    canceldelay_subcommand(CLI::App *actionRoot) {
        ...
        cancel_delay->add_option("canceling_account", canceling_account, localized(
                "Account from authorization on the original delayed transaction"))->required();
        cancel_delay->add_option("canceling_permission", canceling_permission, localized(
                "Permission from authorization on the original delayed transaction"))->required();
        cancel_delay->add_option("trx_id", trx_id,
                                 localized("The transaction id of the original delayed transaction"))->required();
...
void get_account(const string &accountName, bool json_format) {
    ...
    if (!json_format) {
        ...
        std::function<void(account_name, int)> dfs_print = [&](account_name name, int depth) -> void {
            ...
            std::cout << indent << std::string(depth * 3, ' ') << name << ' ' << std::setw(5)
            ...
            for (auto &acc : p.required_auth.accounts) {
                std::cout << sep << acc.weight << ' ' << string(acc.permission.actor) << '@'
                          << string(acc.permission.permission);
                sep = ", ";
            }
            std::cout << std::endl;
        ...
        std::cout << "memory: " << std::endl
                  << indent << "quota: " << std::setw(15) << to_pretty_net(res.ram_quota) << "  used: " << std::setw(15)
                  << to_pretty_net(res.ram_usage) << std::endl << std::endl;
        ...
        std::cout << indent << std::left << std::setw(11) << "used:" << std::right << std::setw(18)
                  << to_pretty_net(res.net_limit.used) << "\n";
        std::cout << indent << std::left << std::setw(11) << "available:" << std::right << std::setw(18)
                  << to_pretty_net(res.net_limit.available) << "\n";
        std::cout << indent << std::left << std::setw(11) << "limit:" << std::right << std::setw(18)
        ...
        std::cout << std::fixed << setprecision(3);
        std::cout << indent << std::left << std::setw(11) << "used:" << std::right << std::setw(18)
                  << to_pretty_time(res.cpu_limit.used) << "\n";
        std::cout << indent << std::left << std::setw(11) << "available:" << std::right << std::setw(18)
        ...
        if (res.refund_request.is_object()) {
            ...
            if (unstaking > asset(0, unstaking.get_symbol())) {
                ...
                std::cout << indent << std::left << std::setw(25) << "time of unstake request:" << std::right
                ...
                std::cout << indent << std::left << std::setw(25) << "from net bandwidth:" << std::right
                          << std::setw(18) << net << std::endl;
                std::cout << indent << std::left << std::setw(25) << "from cpu bandwidth:" << std::right
                          << std::setw(18) << cpu << std::endl;
                std::cout << indent << std::left << std::setw(25) << "total:" << std::right << std::setw(18)
                          << unstaking << std::endl;
                std::cout << std::endl;
            }
        }
...
int main(int argc, char **argv) {
    ...
    app.add_option("-H,--host", obsoleted_option_host_port, localized("the host where nodceles is running"))->group(
            "hidden");
    app.add_option("-p,--port", obsoleted_option_host_port, localized("the port where nodceles is running"))->group(
            "hidden");
    app.add_option("--wallet-host", obsoleted_option_host_port, localized("the host where kcelesd is running"))->group(
            "hidden");
    app.add_option("--wallet-port", obsoleted_option_host_port, localized("the port where kcelesd is running"))->group(
            "hidden");

    app.add_option("-u,--url", url, localized("the http/https URL where nodceles is running"), true);
    app.add_option("--wallet-url", wallet_url, localized("the http/https URL where kcelesd is running"), true);

    app.add_option("-r,--header", header_opt_callback,
                   localized("pass specific HTTP header; repeat this option to pass multiple headers"));
    app.add_flag("-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
    app.add_flag("--no-auto-kcelesd", no_auto_kcelesd,
                 localized("don't automatically launch a kcelesd if one is not currently running"));
    app.set_callback([&app] { ensure_kcelesd_running(&app); });
    ...
    auto create_key = create->add_subcommand("key", localized(
            "Create a new keypair and print the public and private keys"))->set_callback(
            [&r1, &key_file, &print_console]() {
    ...
    // convert subcommand
    auto convert = app.add_subcommand("convert", localized("Pack and unpack transactions"),
                                      false); // TODO also add converting action args based on abi from here ?
    ...
    auto pack_transaction = convert->add_subcommand("pack_transaction",
                                                    localized("From plain signed json to packed form"));
    pack_transaction->add_option("transaction", plain_signed_transaction_json,
                                 localized("The plain signed json (string)"))->required();
    pack_transaction->add_flag("--pack-action-data", pack_action_data_flag,
                               localized("Pack all action data within transaction, needs interaction with nodceles"));
    pack_transaction->set_callback([&] {
        fc::variant trx_var;
        try {
            trx_var = json_from_file_or_string(plain_signed_transaction_json);
        } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse plain transaction JSON '${data}'",
                                 ("data", plain_signed_transaction_json))
        if (pack_action_data_flag) {
            ...
            try {
                ...
                std::cout << fc::json::to_pretty_string(fc::variant(packed_transaction(trx, packed_transaction::none)))
                          << std::endl;
            } EOS_RETHROW_EXCEPTIONS(transaction_type_exception,
    ...
    auto unpack_transaction = convert->add_subcommand("unpack_transaction",
                                                      localized("From packed to plain signed json form"));
    unpack_transaction->add_option("transaction", packed_transaction_json, localized(
    ...
    unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag,
                                 localized("Unpack all action data within transaction, needs interaction with nodceles"));
    unpack_transaction->set_callback([&] {
        ...
        } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse packed transaction JSON '${data}'",
                                 ("data", packed_transaction_json))
    ...
    auto pack_action_data = convert->add_subcommand("pack_action_data",
                                                    localized("From json action data to packed form"));
    pack_action_data->add_option("account", unpacked_action_data_account_string,
                                 localized("The name of the account that hosts the contract"))->required();
    pack_action_data->add_option("name", unpacked_action_data_name_string,
                                 localized("The name of the function that's called by this action"))->required();
    pack_action_data->add_option("unpacked_action_data", unpacked_action_data_string,
                                 localized("The action data expressed as json"))->required();
    pack_action_data->set_callback([&] {
        ...
        bytes packed_action_data_string = variant_to_bin(unpacked_action_data_account_string,
                                                         unpacked_action_data_name_string, unpacked_action_data_json);
    ...
    auto unpack_action_data = convert->add_subcommand("unpack_action_data",
                                                      localized("From packed to json action data form"));
    ...
    unpack_action_data->add_option("packed_action_data", packed_action_data_string,
                                   localized("The action data expressed as packed hex string"))->required();
    unpack_action_data->set_callback([&] {
        ...
        fc::variant unpacked_action_data_json = bin_to_variant(packed_action_data_account_string,
                                                               packed_action_data_name_string, packed_action_data_blob);
    ...
    getCode->add_option("name", accountName,
                        localized("The name of the account whose code should be retrieved"))->required();
    getCode->add_option("-c,--code", codeFilename,
                        localized("The name of the file to save the contract .wast/wasm to"));
    ...
    getCode->set_callback([&] {
        ...
        try {
            const auto result = call(get_raw_code_and_abi_func,
            ...
            //see if this is an old nodceles that doesn't support get_raw_code_and_abi
            const auto old_result = call(get_code_func,
            ...
            if (code_as_wasm) {
                ...
                std::cout << localized("Warning: communicating to older nodceles which returns malformed binary wasm")
                          << std::endl;
            } else
        ...
        if (codeFilename.size()) {
            std::cout << localized("saving ${type} to ${codeFilename}",
                                   ("type", (code_as_wasm ? "wasm" : "wast"))("codeFilename", codeFilename))
                      << std::endl;
    ...
    getAbi->add_option("name", accountName,
                       localized("The name of the account whose abi should be retrieved"))->required();
    getAbi->add_option("-f,--file", filename,
                       localized("The name of the file to save the contract .abi to instead of writing to console"));
    getAbi->set_callback([&] {
    ...
    getTable->add_option("scope", scope,
                         localized("The scope within the contract in which the table is found"))->required();
    getTable->add_option("table", table,
                         localized("The name of the table as specified by the contract abi"))->required();
    getTable->add_option("-b,--binary", binary,
                         localized("Return the value as BINARY rather than using abi to interpret as JSON"));
    ...
    getTable->add_option("-L,--lower", lower,
                         localized("JSON representation of lower bound value of key, defaults to first"));
    getTable->add_option("-U,--upper", upper,
                         localized("JSON representation of upper bound value of key, defaults to last"));
    ...
    string symbol;
    auto get_currency = get->add_subcommand("currency",
                                            localized("Retrieve information related to standard currencies"), true);
    get_currency->require_subcommand();
    auto get_balance = get_currency->add_subcommand("balance", localized(
            "Retrieve the balance of an account for a given currency"), false);
    ...
    get_balance->add_option("symbol", symbol,
                            localized("The symbol for the currency if the contract operates multiple currencies"));
    ...
    auto get_currency_stats = get_currency->add_subcommand("stats",
                                                           localized("Retrieve the stats of for a given currency"),
                                                           false);
    get_currency_stats->add_option("contract", code, localized("The contract that operates the currency"))->required();
    get_currency_stats->add_option("symbol", symbol, localized(
    ...
    auto getAccounts = get->add_subcommand("accounts", localized("Retrieve accounts associated with a public key"),
                                           false);
    getAccounts->add_option("public_key", public_key_str,
                            localized("The public key to retrieve accounts for"))->required();
    ...
    auto getServants = get->add_subcommand("servants",
                                           localized("Retrieve accounts which are servants of a given account "),
                                           false);
    getServants->add_option("account", controllingAccount,
                            localized("The name of the controlling account"))->required();
    ...
    getTransaction->add_option("-b,--block-hint", block_num_hint,
                               localized("the block number this transaction may be in"));
    ...
    int32_t pos_seq = -1;
    int32_t offset = -20;
    auto getActions = get->add_subcommand("actions", localized(
            "Retrieve all actions with specific account name referenced in authorization or receiver"), false);
    ...
    getActions->add_option("offset", offset, localized(
    ...
    getActions->set_callback([&] {
        ...
        if (printjson) {
            ...
            cout << "#" << setw(5) << "seq" << "  " << setw(24) << left << "when" << "  " << setw(24) << right
    ...
    codeSubcommand->add_option("code-file", wasmPath,
                               localized("The fullpath containing the contract WASM"));//->required();
    codeSubcommand->add_flag("-c,--clear", contract_clear, localized("Remove code on an account"));
    codeSubcommand->add_flag("--suppress-duplicate-check", suppress_duplicate_check,
                             localized("Don't check for duplicate"));
    ...
    abiSubcommand->add_option("abi-file", abiPath,
                              localized("The fullpath containing the contract ABI"));//->required();
    abiSubcommand->add_flag("-c,--clear", contract_clear, localized("Remove abi on an account"));
    abiSubcommand->add_flag("--suppress-duplicate-check", suppress_duplicate_check,
                            localized("Don't check for duplicate"));
    ...
    contractSubcommand->add_option("wasm-file", wasmPath,
                                   localized("The file containing the contract WASM relative to contract-dir"));
...
//                                ->check(CLI::ExistingFile);
    ...
    contractSubcommand->add_flag("--suppress-duplicate-check", suppress_duplicate_check,
                                 localized("Don't check for duplicate"));
    ...
    auto set_code_callback = [&]() {
        ...
        if (!contract_clear) {
            ...
            if (wasm.compare(0, 8, binary_wasm_header))
                std::cerr << localized("WARNING: ") << wasmPath << localized(
                        " doesn't look like a binary WASM file. Is it something else, like WAST? Trying anyways...")
                          << std::endl;
    ...
    // set account
    auto setAccount = setSubcommand->add_subcommand("account", localized(
    ...
    // set action
    auto setAction = setSubcommand->add_subcommand("action", localized(
    ...
    string memo;
    bool pay_ram = false;
    auto transfer = app.add_subcommand("transfer", localized("Transfer CCH from account to account"), false);
    ...
    createWallet->add_option("-f,--file", password_file, localized(
    ...
    importWallet->set_callback([&wallet_name, &wallet_key_str] {
        ...
        try {
            ...
            EOS_THROW(private_key_type_exception, "Invalid private key: ${private_key}",
                      ("private_key", wallet_key_str))
        }
    ...
    createKeyInWallet->add_option("-n,--name", wallet_name, localized("The name of the wallet to create key into"),
                                  true);
    createKeyInWallet->add_option("key_type", wallet_create_key_type, localized("Key type to create (K1/R1)"),
                                  true)->set_type_name("K1/R1");
    createKeyInWallet->set_callback([&wallet_name, &wallet_create_key_type] {
        ...
        std::cout << localized("Created new private key with a public key of: ") << fc::json::to_pretty_string(v)
                  << std::endl;
    });
    ...
    // list private keys
    auto listPrivKeys = wallet->add_subcommand("private_keys", localized(
            "List of private keys from an unlocked wallet in wif or PVT_R1 format."), false);
    ...
    auto stopKcelesd = wallet->add_subcommand("stop", localized("Stop kcelesd (doesn't work with nodceles)."), false);
    stopKcelesd->set_callback([] {
    ...
    sign->add_option("-k,--private-key", str_private_key,
                     localized("The private key that will be used to sign the transaction"));
    sign->add_option("-c,--chain-id", str_chain_id,
                     localized("The chain id that will be used to sign the transaction"));
    ...
    sign->set_callback([&] {
        ...
        if (str_chain_id.size() == 0) {
            ilog("grabbing chain_id from nodceles");
            auto info = get_info();
    ...
    actionsSubcommand->add_option("action", action,
                                  localized("A JSON string or filename defining the action to execute on the contract"),
                                  true)->required();
    ...
    actionsSubcommand->set_callback([&] {
        ...
        send_actions({chain::action{accountPermissions, contract_account, action,
    ...
    string trxsJson;
    auto trxsSubcommand = push->add_subcommand("transactions",
                                               localized("Push an array of arbitrary JSON transactions"));
    trxsSubcommand->add_option("transactions", trxsJson, localized(
    ...
    propose_action->add_option("trx_permissions", transaction_perm,
                               localized("The JSON string or filename defining transaction permissions"))->required();
    propose_action->add_option("contract", proposed_contract,
                               localized("contract to which deferred transaction should be delivered"))->required();
    ...
    propose_action->set_callback([&] {
        ...
        } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'",
                                 ("data", requested_perm))
        fc::variant transaction_perm_var;
        try {
            transaction_perm_var = json_from_file_or_string(transaction_perm);
        } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'",
                                 ("data", transaction_perm))
        fc::variant trx_var;
        try {
        ...
        if (accountPermissions.empty()) {
            if (!proposer.empty()) {
                ...
                EOS_THROW(missing_auth_exception,
    ...
    executer = "";
    string trx_to_exec;
    auto sudo_exec = sudo->add_subcommand("exec",
                                          localized("Execute a transaction while bypassing authorization checks"));
    add_standard_transaction_options(sudo_exec);
    sudo_exec->add_option("executer", executer, localized(
            "Account executing the transaction and paying for the deferred transaction RAM"))->required();
    sudo_exec->add_option("transaction", trx_to_exec,
                          localized("The JSON string or filename defining the transaction to execute"))->required();
