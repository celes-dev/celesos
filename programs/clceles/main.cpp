 ...
 *  @brief Tool for sending transactions and querying state from @ref nodceles
...
/**
  ...
  @section intro Introduction to clceles

  `clceles` is a command line tool that interfaces with the REST api exposed by @ref nodceles. In order to use `clceles` you will need to
  have a local copy of `nodceles` running and configured to load the 'eosio::chain_api_plugin'.

   clceles contains documentation for all of its commands. For a list of all commands known to clceles, simply run it with no arguments:
```
$ ./clceles
Command Line Interface to EOSIO Client
Usage: programs/clceles/clceles [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodceles is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kcelesd is running
...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      if(url == ::url)
         std::cerr << localized("Failed to connect to nodceles at ${u}; is nodceles running?", ("u", url)) << std::endl;
      ...
      permissions->set_callback([this] {
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::celes_code_name);
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of CELES delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of CELES delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_celes,
                                   (localized("The amount of RAM bytes to purchase for the new account in CELES")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake CELES to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            if (!simple) {
               EOSC_ASSERT( buy_ram_celes.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
               EOSC_ASSERT( !buy_ram_bytes_in_kbytes || !buy_ram_bytes, "ERROR: --buy-ram-kbytes and --buy-ram-bytes cannot be set at the same time" );
               action buyram = !buy_ram_celes.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_celes))
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of CELES to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of CELES to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of CELES to buyram"));
      delegate_bandwidth->add_option("--buy-ram-bytes", buy_ram_bytes, localized("The amount of RAM to buy in number of bytes"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake CELES to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of CELES to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of CELES to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of CELES to bid"))->required();
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of CELES to pay for RAM, or number of bytes/kibibytes of RAM if --bytes/--kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive CELES for sold RAM"))->required();
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodceles responding to the request is different than core symbol built into clceles
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kcelesd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kcelesd is running") )->group("hidden");
   ...
   app.add_flag( "--no-auto-kcelesd", no_auto_kcelesd, localized("don't automatically launch a kcelesd if one is not currently running"));
   ...
   pack_transaction->add_flag("--pack-action-data", pack_action_data_flag, localized("Pack all action data within transaction, needs interaction with nodceles"));
   ...
   unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag, localized("Unpack all action data within transaction, needs interaction with nodceles"));
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older nodceles which returns malformed binary wasm") << std::endl;
         }
         else
...
//                                ->check(CLI::ExistingFile);
   ...
   string memo;
   bool pay_ram = false;
   auto transfer = app.add_subcommand("transfer", localized("Transfer CELES from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending CELES"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving CELES"))->required();
   transfer->add_option("amount", amount, localized("The amount of CELES to send"))->required();
   ...
   review->set_callback([&] {
      auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                         ("code", "celes.msig")
                         ("scope", proposer)
                         ("table", "proposal")
                         ("table_key", "")
