#!/bin/bash
#
# celesos-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# celesos-tn_bounce.sh and celesos-tn_roll.sh scripts.

connected="0"

rundir=programs/nodceles
prog=nodceles
...
datadir=var/lib/node_$CELESOS_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/celesos/node_$CELESOS_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/celesos/node_$CELESOS_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
...
if [ -z "$CELESOS_LEVEL" ]; then
    echo starting with no modifiers
    relaunch
    if [ "$connected" -eq 0 ]; then
        CELESOS_LEVEL=replay
    else
        exit 0
    fi
fi

if [ "$CELESOS_LEVEL" == replay ]; then
    echo starting with replay
    relaunch --hard-replay-blockchain
    if [  "$connected" -eq 0 ]; then
        CELESOS_LEVEL=resync
    else
        exit 0
    fi
fi
if [ "$CELESOS_LEVEL" == resync ]; then
