...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(celes.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(celes.token), celes_token_wast);
         chain->set_abi(N(celes.token), celes_token_abi);
         chain->push_action( N(celes.token), N(create), N(celes.token), mvo()
              ( "issuer", "celes.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(celes.token), N(issue), N(celes.token), mvo()
              ( "to", "celes.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(celes.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(celes.token), N(alice)};
   test.init();

   test.transfer( N(celes.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(celes.token), N(bob)};
   test.init();

   test.transfer( N(celes.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(celes.token) );
   ...
   test.chain->set_code(N(bob), celes_token_wast);
   test.chain->set_abi(N(bob), celes_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), celes_token_wast);
   test.chain->set_abi(N(charlie), celes_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(celes.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, celes_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auth.accounts.push_back( permission_level_weight{{N(alice), config::celes_code_name}, 1} );
   ...
   auth.accounts.push_back( permission_level_weight{{N(alice), config::celes_code_name}, 1} );
   auth.accounts.push_back( permission_level_weight{{N(bob), config::celes_code_name}, 1} );
   ...
   auth.accounts.push_back( permission_level_weight{{N(charlie), config::celes_code_name}, 1} );
