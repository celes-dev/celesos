...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(celes.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(celes.token), celes_token_wast );
   c.set_abi( N(celes.token), celes_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(celes.token), N(create), N(celes.token), mutable_variant_object()
              ("issuer",       "eosio" )
   ...
   cr = c.push_action( N(celes.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
