...
#include <celes.sudo/celes.sudo.wast.hpp>
#include <celes.sudo/celes.sudo.abi.hpp>
...
class celes_sudo_tester : public tester {
public:

   celes_sudo_tester() {
      create_accounts( { N(celes.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(celes.sudo), celes_sudo_wast, &system_private_key );
      set_abi( N(celes.sudo), celes_sudo_abi, &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(celes.sudo) );
      abi_def abi;
   ...
   void propose( name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx ) {
      push_action( N(celes.msig), N(propose), proposer, mvo()
                     ("proposer",      proposer)
   ...
   void approve( name proposer, name proposal_name, name approver ) {
      push_action( N(celes.msig), N(approve), approver, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
   ...
   void unapprove( name proposer, name proposal_name, name unapprover ) {
      push_action( N(celes.msig), N(unapprove), unapprover, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
...
transaction celes_sudo_tester::sudo_exec( account_name executer, const transaction& trx, uint32_t expiration ) {
   fc::variants v;
...
transaction celes_sudo_tester::reqauth( account_name from, const vector<permission_level>& auths, uint32_t expiration ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(celes_sudo_tests)

BOOST_FIXTURE_TEST_CASE( sudo_exec_direct, celes_sudo_tester ) try {
...

   {
      ...
      sudo_trx.actions.emplace_back( get_action( N(celes.sudo), N(exec),
                                                 {{N(alice), config::active_name}, {N(celes.sudo), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig, celes_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   ...
   push_action( N(celes.msig), N(exec), N(alice), mvo()
                  ("proposer",      "carol")
                  ("proposal_name", "first")
                  ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( "celes.sudo", name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_unapprove, celes_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   auto sudo_trx = sudo_exec( N(alice), trx );
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of celes.sudo::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(celes.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_producers_change, celes_sudo_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of celes.sudo::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(celes.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
